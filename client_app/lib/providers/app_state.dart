...
import 'package:taxi_client/models/driver.dart';
import 'package:taxi_client/models/ride_Request.dart';
import 'package:taxi_client/models/route.dart';
import 'package:taxi_client/models/user.dart';
import 'package:taxi_client/services/drivers.dart';
import 'package:taxi_client/services/map_requests.dart';
import 'package:taxi_client/services/ride_requests.dart';
import 'package:taxi_client/widgets/custom_btn.dart';
import 'package:taxi_client/widgets/custom_text.dart';
import 'package:taxi_client/widgets/stars.dart';
  ...
  requestDriver(
    ...
    await dioRespone.post(
      ...
      data: {
        'client_id': user.dbID,
        'start_pos': "$lat : $lng",
        'end_pos': "$requestedDestinationLat : $requestedDestinationLng",
        'distance': distance['value'],
        'price': distance['value'],
        'category': 'Комфорт',
        'number_seats': 4,
      },
    ).then((value) {
      print(value.data['callr_id']);
      callr_id = value.data['callr_id'].toString();
    });
    ...
    percentageCounter(requestId: id, requestDbId: callr_id, context: context);
  }
...
// ANCHOR LISTEN TO DRIVER
  ...
  percentageCounter(
      {String requestId, String requestDbId, BuildContext context}) {
    lookingForDriver = true;
    notifyListeners();
    periodicTimer = Timer.periodic(Duration(seconds: 1), (time) async {
